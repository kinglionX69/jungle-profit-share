...
const Header: React.FC = () => {
  ...
  return <header className="bg-background/95 backdrop-blur-md sticky top-0 z-40 border-b">
      <nav className="mx-auto flex max-w-7xl items-center justify-between p-4 lg:px-8" aria-label="Global">
        <div className="flex lg:flex-1">
          <a href="/" className="-m-1.5 p-1.5 flex items-center gap-2">
            <span className="h-8 w-8 bg-primary rounded-md flex items-center justify-center">
              <span className="text-white font-bold text-lg">PLS</span>
            </span>
            <span className="font-semibold text-xl">Proud Lion Studios Pride Share</span>
          </a>
        </div>
        ...
        <div className="hidden lg:flex lg:gap-x-12">
          ...
          e.preventDefault();
          navigate(item.href);
        }} className={`text-sm font-semibold leading-6 relative transition-all hover:text-primary ${isCurrentPage(item.href) ? 'text-primary after:absolute after:bottom-0 after:left-0 after:w-full after:h-0.5 after:bg-primary' : 'text-foreground'}`}>
              {item.name}
            </a>)}
        </div>
        
        <div className="hidden lg:flex lg:flex-1 lg:justify-end">
          {connected ? <DropdownMenu>
            ...
            </DropdownMenu> : <Button onClick={connect} disabled={connecting} className="bg-primary hover:bg-primary/90">
      ...
      {mobileMenuOpen && <div className="lg:hidden">
          ...
          <div className="fixed inset-y-0 right-0 z-50 w-full overflow-y-auto bg-background px-6 py-6 sm:max-w-sm sm:ring-1 sm:ring-gray-900/10">
            ...
            <div className="mt-6 flow-root">
              ...
              }} className={`-mx-3 block rounded-lg px-3 py-2 text-base font-semibold leading-7 ${isCurrentPage(item.href) ? 'text-primary bg-muted' : 'text-foreground hover:bg-muted'}`}>
                      {item.name}
                    </a>)}
                </div>
                <div className="py-6">
                  {connected ? <div className="space-y-2">
                      ...
                      <Button variant="outline" className="w-full justify-start text-destructive mt-2" onClick={disconnect}>
                        <LogOut className="h-4 w-4 mr-2" />
                        Disconnect
                      </Button>
                    </div> : <Button className="w-full" onClick={() => {
                connect();
                setMobileMenuOpen(false);
              }} disabled={connecting}>
                      {connecting ? 'Connecting...' : 'Connect Wallet'}
